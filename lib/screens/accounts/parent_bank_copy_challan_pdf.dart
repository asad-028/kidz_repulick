...
class _DocumentListState extends State<DocumentList> {
  ...
  Future<void> _fetchDocuments(condition2) async {
    try {
      ...
      (role_ == 'Manager')
          ? condition = ''
          : condition = "'fathersemail',isEqualTo: ${user!.email}";
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          SizedBox(
                              width:
                                  10.0), // Spacing between content and divider
                          (_selectedCategory != 'Not Paid')
                              ? Container(
                                  width: mq.width * 0.10,
                                  child: CachedNetworkImage(
                                      imageUrl: document['paymentProof']))
                              : Container()
                        ],
                      ),
                      Container(
...
class _PrepareBankCopyFromFirebasePDFState
            ...
            ? Row(
                ...
                children: [
                  ...
                  ElevatedButton.icon(
                    onPressed: () async {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          ...
                          content: Text(
                            'Account Number ${bankController.accountNumber.value} Copied to clip board',
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor:
                          Colors.green[50], // Customize button color
                    ),
                  ),
                        ...
                        : ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              ...
                              content: Wrap(children: [
                                ...
                                Text(
                                  'Payment proof not uploaded, Unable to verify',
                                  style: TextStyle(color: Colors.black),
                                )
                              ]),
                            ),
            ...
            child: Column(
              ...
              children: [
                      ...
                      child: Obx(() {
                           if (bankController.bankImage.value.isNotEmpty) {
                               return CachedNetworkImage(
                                   ...
                                   fit: BoxFit.contain,
                                   placeholder: (context, url) => Image.asset('assets/bank_icon.png'),
                                   errorWidget: (context, url, error) => Image.asset('assets/bank_icon.png'),
                               );
                           } else {
                               return Image(
                ...
                Row(
                  children: [
                    Text(
                        "Credit:              ${bankController.creditTo.value}"),
                  ],
                ),
                Row(
                  children: [
  ...
  Future<void> _generatePdf() async {
    ...
    // Load default bank icon
    final imageData = await rootBundle.load('assets/bank_icon.png');
    final imageBytes = imageData.buffer.asUint8List();
    pw.MemoryImage imagebank = pw.MemoryImage(imageBytes);

    // Try detecting if custom bank image is available
    if (bankController.bankImage.value.isNotEmpty) {
        try {
            final customBankImage = await networkImage(bankController.bankImage.value);
            // networkImage returns ImageProvider, but pw.Image takes ImageProvider
            // actually pw.Image takes an ImageProvider
            // But I was using pw.MemoryImage before.
            // checking printing package docs: networkImage returns 'ImageProvider' which is compatible
            // Let's try to assign it to imagebank, but imagebank type was inferred or set as MemoryImage?
            // I'll just change the variable type to pw.ImageProvider
             // Wait, networkImage is from 'package:printing/printing.dart';
             // It returns Future<ImageProvider>
             imagebank = customBankImage as pw.MemoryImage; // Verify if this works. networkImage usually returns something compatible with pdf package.
             // Actually, printing.networkImage returns Future<ImageProvider> where ImageProvider is from pdf/widgets.dart
             // Let's see. 
             // Error: A value of type 'ImageProvider' can't be assigned to a variable of type 'MemoryImage'.
             // So I should declare imagebank as pw.ImageProvider
        } catch (e) {
            print("Error loading network image for PDF: $e");
            // Fallback to default
        }
    }
    ...
    // To handle imagebank type consistently:
    pw.ImageProvider bankImageProvider = imagebank;
    
    // Check for cached bytes first (fastest)
    if (bankController.bankImageBytes.value != null) {
        bankImageProvider = pw.MemoryImage(bankController.bankImageBytes.value!);
    } 
    // Fallback to URL if bytes not cached but URL exists
    else if (bankController.bankImage.value.isNotEmpty) {
        try {
           bankImageProvider = await networkImage(bankController.bankImage.value);
        } catch (e) {
            print("Error loading custom bank image for PDF: $e");
        }
    }
      ...
      pw.Page(
        ...
        build: (pw.Context context) {
          return pw.Container(
            ...
            crossAxisAlignment: pw.CrossAxisAlignment.center,
            children: [
              _buildContent('Bank Copy', bankImageProvider, imagekrdc),
              pw.SizedBox(width: 5),
              _buildContent('School Copy', bankImageProvider, imagekrdc),
              pw.SizedBox(width: 5),
              _buildContent('Parents Copy', bankImageProvider, imagekrdc),
            ],
          ));
        },
      ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                      width: 300, height: 400, child: _buildImagePreview()),
                  ElevatedButton.icon(
                    onPressed: () async {
                      if (_formKey.currentState!.validate()) {
                        _formKey.currentState!.save(); // Save form data
                        await uploadPaymentProofDetails(context, documentId,
                            amountPaid, paymentId, pickedImage);
                      }
                    },
  ...
  Future<void> uploadPaymentProofDetails(
    ...
    showDialog(
      context: context,
      barrierDismissible:
          false, // Prevent user from closing dialog while uploading
    ...
    try {
      ...
      final storageRef =
          FirebaseStorage.instance.ref().child('paymentproofs/$fileName');
